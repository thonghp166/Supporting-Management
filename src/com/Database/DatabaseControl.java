...
public class DatabaseControl implements Database {
    ...
    public void clearTable(String tableName) {
        ArrayList<String> colName = getColumnNames(tableName);
        ArrayList<String> colType = new ArrayList<>();
        for (int i=0; i<colName.size(); i++) colType.add("Tự do");
        dropTable(tableName);
        createTable(tableName, colName, colType);
        String clear = "TRUNCATE TABLE `" + tableName + "`";
        callConstantQuery(clear);
        System.out.println("Clear Table "+tableName+" Successfully!");
    }
    ...
    public ArrayList<ArrayList<String>> getDataOfTable(String tableName) {
        ArrayList<String> colNames = getColumnNames(tableName);
        ArrayList<ArrayList<String>> preData = new ArrayList<>();

        for (String name : colNames) {
            ArrayList<String> columnData = getStringOfColumn(tableName, name);
            preData.add(columnData);
        }

        ArrayList<ArrayList<String>> data = new ArrayList<>();
        for (int i=0; i<preData.get(0).size(); i++) {
            ArrayList<String> row = new ArrayList<>();
            for (int j = 0; j < preData.size(); j++) row.add(preData.get(j).get(i));
            data.add(row);
        }
    ...
    public ArrayList<String> getListContents(String year) {
        String tableName = year + "#" + "contents";
    ...
    public ArrayList<ArrayList<String>> getData(String year, String month, String Subcontent) {
        ...
        if (!month.equals("Cả năm")) tableName += month + "#";
        tableName+= getIndexOfContent(year, parentContent);
    ...
    public void updateDataOfTable(String tableName, ArrayList<ArrayList<String>> data) {
        ...
        ArrayList<String> contents = getStringOfColumn(tableName, "Nội dung");
        for (int i= 0; i<data.size(); i++) data.get(i).add(0, contents.get(i));
